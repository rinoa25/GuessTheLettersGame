...
# Sees if user inputs exit/stop then game terminates, if not checks if the input is a number and calls the
# checkGuessNumRange.
...
def askUserToGuess():
    ...
    global guessesLeft
    global userGuess
    if terminationWanted:
        print()
        askToReplay()
    elif not terminationWanted:
        if not gameFinish:
...
def showLettersUsed():
    ...
    if userGuess.lower() in ['exit', 'stop']:
        ...
        if userGuess.lower() in fixedWord and gameFinish == False and lettersFound == False:
        ...
        if not userGuess.lower() in fixedWord and gameFinish == False and lettersFound == False:
...
def repeatedLetters():
    ...
    if userGuess.lower() in ['exit', 'stop']:
        ...
        if userGuess.lower() in wrongLetters and not gameFinish and not lettersFound:
            ...
            if userGuess.lower() in ['exit', 'stop']:
                ...
                verifyUserGuess()
        if userGuess.lower() in correctLetters and not gameFinish and not lettersFound:
